...
3. **Performance**: EF N+1 / missing `AsNoTracking` / tracking on read paths / client-side evaluation, missing caching on read-heavy paths or wrong FusionCache usage (manual get+set instead of `GetOrSetAsync`, missing invalidation on writes, caching entities instead of DTOs), allocations in hot loops, reflection-based JSON where a `JsonSerializerContext` exists, missing pagination on unbounded queries.
