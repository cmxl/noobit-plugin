...
Load these skills first and follow them exactly (they ship with the noobit plugin — fully qualified names are `noobit:<name>`): `aspnet-backend`, `bff-security`, `fusioncache-redis`, `data-access` plus the matching provider skill (`mssql`/`postgres`/`sqlite`), `docker`, `nginx-deploy`, `dotnet-testing`, `docs-maintenance`. For the frontend also load `noobit:angular-ngrx-state` (ships with this plugin) and `angular-new-app` (**external** — official Angular Team skill; if it is not installed, scaffold the Angular app with the Angular CLI directly and note the missing skill). Use my stack defaults throughout; do not ask configuration questions. When pinning package versions in `Directory.Packages.props` and `package.json`, check the current stable versions (nuget.org / npm / the official docs listed in the skills) instead of relying on memory.

Invoking this command **is** the approved design: the stack defaults are the decisions, so skip superpowers brainstorming/design-approval gates and proceed straight to scaffolding.
...
6. **Hosting**: Dockerfile and `compose.yaml` per `docker` (multi-stage .NET+Angular, non-root, cache mounts; app, db, redis[, rabbitmq]) with the nginx + certbot services and nginx conf per `nginx-deploy`, `compose.override.yaml` for local dev (published DB/redis ports, no nginx/certbot), `.env.example`, `.gitignore`, and an `.editorconfig` that mechanically enforces the C# naming/style rules (`dotnet_naming` rules: PascalCase types/methods/properties/constants, `_camelCase` private fields, `Async` suffix on async methods, interfaces `I`-prefixed; plus file-scoped namespaces, `System` usings first, one type per file) — style lives in tooling, not prose.
