...
Load these skills first and follow them exactly (they ship with the noobit plugin — fully qualified names are `noobit:<name>`): `aspnet-backend`, `bff-security`, `fusioncache-redis`, `data-access`, `docker-nginx-deploy`, `dotnet-testing`, `docs-maintenance` — plus `angular-new-app` and `angular-ngrx-state` for the frontend if they are installed. Use my stack defaults throughout; do not ask configuration questions.

Create:

1. **Solution layout**: `src/<Name>.Api` (net10.0 BFF host, feature folders, minimal APIs, ProblemDetails, health checks, OpenTelemetry stub), `src/<Name>.Domain`, `tests/<Name>.Tests`, `tests/<Name>.IntegrationTests` (ApiFixture with Testcontainers + Respawn, one passing smoke test). Repo-root build files per `aspnet-backend`: `global.json` (SDK pin), `Directory.Build.props` (Nullable, ImplicitUsings, TreatWarningsAsErrors, LangVersion latest), `Directory.Packages.props` (central package management — all versions there, csproj references version-less), and `Directory.Build.rsp` containing at least `-maxcpucount`, `-nologo`, `-graph`.
